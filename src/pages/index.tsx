...
export default function Home() {
  ...
  // Providers data
  // Using JSON-like arrays defined in utils for easy editing
  // Lazy import to avoid circular issues with Next.js SSR
  const { featuredProviders, moreProviders } = require("../utils/providers");

  // Stagger animation settings for provider cards
  const PROVIDER_BASE_DELAY = 0.1; // seconds
  const PROVIDER_STAGGER = 0.1; // seconds per card
        ...
        <div className="flex dir-column">
          <div className="flex v-center gap-sm">
            ...
            <div className="icons">
              ...
              <Image width={50} height={50} className="icon" src="/check.png" alt="" />
            </div>
          </div>
        ...
        <section id="providers">
          <div className="wrapper fixed">
            ...
            <div className="providers-container">
              <div className="row">
                {featuredProviders.map((p: any, idx: number) => (
                  <div key={`${p.name}-${idx}`} className="col-md-3 col-tb-6 col-lp-4">
                    {renderProviderCard(
                      p.name,
                      p.image,
                      p.twitterUrl,
                      p.description,
                      PROVIDER_BASE_DELAY + idx * PROVIDER_STAGGER
                    )}
                  </div>
                ))}
                {/* Legacy hardcoded cards retained below for reference; now replaced by JSON-driven list
              ...
              {showAllProviders && (
                <div className="additional-providers">
                  <div className="row">
                    {moreProviders.map((p: any, idx: number) => (
                      <div key={`${p.name}-${idx}`} className="col-md-3 col-tb-6 col-lp-4">
                        {renderProviderCard(
                          p.name,
                          p.image,
                    ...
                    {/* Additional legacy cards removed */}
                  </div>
                </div>
              )}
            </div>
